...
public class TestPlayer4 : PlayerBase {
	...
	public WheelAttack attackObject;
	public SpecialMove2 spObject;

	public float touchTime = 0f;

	Vector2 feverTouchOrigin;
	...
	public override void Initialize()
	{
		...
		attackObject.tp.position = tp.position;
...
//		attackObject.progressCheck = GetFeverEnabled();
	...
	public void TouchCheck()
	{
		...
		else if(Input.GetMouseButton(0) && isAttack && GetFeverEnabled())
		{
			...
			if(touchTime <= 0.3f)
			{
				...
				if(Vector2.Distance(feverTouchOrigin,pos) > 3f
					&& pos.x > feverTouchOrigin.x)
				{
					attackObject.Active(tp.position);
				}
			}
			// Vector3 pos = Camera.main.ScreenToWorldPoint(Input.mousePosition);
			// pos.z = 0;
	...
	public override void Attack()
	{
		...
		// GameObjectManager.instance.bulletManager.ObjectActive(tp.position,40f,1f,0f,BulletBase.BulletTeam.Player);
		// GameObjectManager.instance.bulletManager.ObjectActive(tp.position,40f,1f,-2.5f,BulletBase.BulletTeam.Player);
		// GameObjectManager.instance.bulletManager.ObjectActive(tp.position,40f,1f,-5f,BulletBase.BulletTeam.Player);
	}
	public override void FeverAttack(Vector2 touchPos)
	{
		//attackObject.Aiming(touchPos);
	}
